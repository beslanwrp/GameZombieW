...
Seis pantallas estáticas en la dirección «crudo fotorealista» con cartas tipo expediente y polaroid. Se generan con `node gen.mjs` (escribe los `.dc.html` a partir de `_base.css` y `board.mjs`) y se publican como lienzo editable de Claude Design.
...
| `Carta.dc.html` | Carta de objeto a pantalla completa (polaroid con clip y nota a máquina) |
...
Paleta: asfalto `#141614`, superficie `#1d201c`, oliva `#4f5a3a`, óxido `#c9553d`, cinta de peligro `#e0b43a`, papel de expediente `#e9e2cf`. Tipografías: Big Shoulders Display (titulares), Special Elite (expedientes y notas a máquina), Source Sans 3 (interfaz). Los iconos son SVG de trazo, sin emoji.

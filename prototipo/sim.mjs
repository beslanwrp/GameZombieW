...
function objetivoBot(G, j) {
  ...
  switch (M.objetivo) {
    ...
    case 'sobrevivir': if (M.enRefugio && G.ronda >= G.rondasMax - 2) { const radio = M.enRefugio === true ? 0 : M.enRefugio; if (R.dist(j.pos, '0,0') > radio) { const libre = C.filter(c => R.dist(c.k, '0,0') <= radio && c.revelada && !R.zombisEn(G, c.k).length).sort((a, b) => R.dist(a.k, j.pos) - R.dist(b.k, j.pos))[0]; return libre ? libre.k : '0,0'; } return j.pos; } // falls through

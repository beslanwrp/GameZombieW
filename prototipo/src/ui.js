// Z-2099 · interfaz del prototipo M0 (un dispositivo, se pasa el móvil).
import { PARAMS, OBJETOS, RECETAS, ZOMBIS, PERSONAJES, MISIONES } from './datos.js';
import { nuevaPartida, turnoActual, destinosPosibles, mover, rastrear, saquear, objetivosAtaque, atacar, armasDe, craftear, curar, objetivosCura, triaje, levantar, dar, usar, vehiculo, descansar, terminarTurno, repetirDado, zMoverHorda, zMover, zAtacar, zOler, zTerminar, fichaZombi, zombisEn, jugadoresEn, vecinos, dist, pixel, esHorda, vivos, peso, capacidad, comprobarFin } from './reglas.js';

const $ = s => document.querySelector(s);
const el = (tag, attrs = {}, ...kids) => { const e = document.createElement(tag); for (const [k, v] of Object.entries(attrs)) { if (k === 'class') e.className = v; else if (k.startsWith('on')) e.addEventListener(k.slice(2), v); else if (k === 'html') e.innerHTML = v; else e.setAttribute(k, v); } kids.flat().forEach(k => k != null && e.append(k.nodeType ? k : document.createTextNode(k))); return e; };
const CLAVE = 'z2099-m0';
let G = null, UI = { pantalla: 'inicio', reparto: null, mostrado: null, modo: 'mover', sel: null, zSel: null, cam: { x: 0, y: 0, s: 1 }, verLog: false };
...
function cargar() { try { const d = JSON.parse(localStorage.getItem(CLAVE)); if (d && d.G) { G = d.G; Object.assign(UI, d.UI); return true; } } catch (e) { } return false; }
...
function avisos(cb) { if (!G || !G.avisos.length) { cb && cb(); return; } const a = G.avisos.shift(); modal(a.titulo, `<p>${a.texto}</p>`, [['Seguir', () => avisos(cb)]], a.tipo); }
function toast(t) { const x = $('#toast'); x.textContent = t; x.hidden = false; clearTimeout(x._t); x._t = setTimeout(() => x.hidden = true, 2200); }

/* ---------- inicio ---------- */
function inicio() {
  const cont = $('#p-inicio .cont'); cont.innerHTML = '';
  const hay = (() => { try { return !!localStorage.getItem(CLAVE); } catch (e) { return false; } })();
  if (hay) cont.append(el('button', { class: 'primario ancho', onclick: () => { if (cargar()) reanudar(); } }, 'Continuar la partida guardada'));
  const n = el('select', {}, ...[2, 3, 4, 5, 6].map(i => el('option', { value: i }, i + ' jugadores')));
  n.value = 4; const nombres = el('div', { class: 'nombres' });
  const pintaNombres = () => { nombres.innerHTML = ''; for (let i = 0; i < +n.value; i++) nombres.append(el('input', { type: 'text', placeholder: 'Jugador ' + (i + 1), maxlength: 14, value: '' })); };
  ...
  cont.append(el('label', {}, 'Jugadores', n), nombres, el('label', {}, 'Misión', mis),
    el('button', { class: 'primario ancho', onclick: () => {
      ...
      const pool = Object.keys(PERSONAJES).sort(() => Math.random() - .5);
      UI.reparto = { jugadores: js, misionId, idx: 0, ofertas: js.map((_, i) => [pool[(i * 2) % 6], pool[(i * 2 + 1) % 6]]), semilla: Math.floor(Math.random() * 1e9) };
      // con 4 o más jugadores, las ofertas se solapan: se recalculan quitando los ya elegidos al ir eligiendo
      reparto();
    } }, 'Empezar'));
  mostrar('inicio');
}
function reparto() {
  const Rp = UI.reparto; if (Rp.idx >= Rp.jugadores.length) { G = nuevaPartida({ jugadores: Rp.jugadores, misionId: Rp.misionId, semilla: Rp.semilla }); UI.reparto = null; UI.mostrado = null; guardar(); modal('Misión: ' + MISIONES[G.misionId].nombre, `<p>${MISIONES[G.misionId].texto}</p><p class="muted">Contagio: ${etiquetaContagio(G.misionId)}. Límite: ${MISIONES[G.misionId].rondas} rondas.</p>`, [['A la mesa', () => reanudar()]]); return; }
  ...
  const oferta = libres.slice(0, Math.min(2, libres.length));
  pasar(j.nombre, null, () => {
    ...
    oferta.forEach(pid => { const P = PERSONAJES[pid]; c.append(el('button', { class: 'carta-personaje', style: `--c:${P.color}`, onclick: () => { j.personajeId = pid; Rp.idx++; reparto(); } },
      el('div', { class: 'cp-nombre' }, P.nombre, el('span', {}, P.alias)), el('div', { class: 'cp-stats' }, `Vida ${P.vida} · Iniciativa ${P.iniciativa} · Capacidad ${P.capacidad} · Dados ${P.dados}`),
...
function etiquetaContagio(id) { return { cuenta_atras: 'cuenta atrás de 4 turnos', sin_contagio: 'sin contagio', hardcore: 'hardcore' }[MISIONES[id].contagio]; }
...
/* ---------- bucle principal ---------- */
function reanudar() {
  if (!G) return inicio(); if (G.fin) return fin();
  avisos(() => {
...
function cabecera(root) {
  ...
  const ruido = el('div', { class: 'ruido' + (G.ruido >= 6 ? ' alto' : ''), title: 'Ruido' }, el('span', { class: 'lab' }, 'Ruido'), el('div', { class: 'ruido-bar' }, ...Array.from({ length: PARAMS.ruidoTope }, (_, i) => el('i', { class: i < G.ruido ? 'on' : '' }))), el('b', {}, `${G.ruido}/${PARAMS.ruidoTope}`));
  const obj = M.objetivo === 'antibioticos_refugio' ? `Antibióticos en refugio ${G.almacen.antibioticos}/${M.cantidad}` : M.objetivo === 'comida_refugio' ? `Comida en refugio ${G.almacen.comida}/${M.cantidad}` : M.objetivo === 'todos_helipuerto' ? 'Todos al helipuerto (H)' : `Sobrevivir con al menos ${Math.ceil(G.jugadores.length / 2)}`;
  h.append(el('div', { class: 'mision' }, el('b', {}, M.nombre), el('span', {}, ` · Ronda ${G.ronda}/${M.rondas} · ${obj}`)), ruido, el('button', { class: 'mini', onclick: verLog }, 'Registro'));
}
...
function pintarJuego() {
  ...
  const estado = [j.mordido ? `<span class="tag peligro">Mordido${j.mordido.turnos != null ? ' · ' + j.mordido.turnos + ' turnos' : ''}</span>` : '', j.panico ? `<span class="tag">Pánico ${j.panico}</span>` : '', j.camuflaje ? `<span class="tag bien">Camuflaje ${j.camuflaje}</span>` : '', j.vehiculo ? `<span class="tag bien">${OBJETOS[j.vehiculo.id].nombre} · gas ${j.vehiculo.gas}</span>` : '', peso(G, j) > capacidad(G, j) ? '<span class="tag peligro">Sobrecarga</span>' : ''].join('');
  panel.append(el('div', { class: 'quien' }, el('b', {}, j.nombre), el('span', {}, ` · ${P.nombre}, ${P.alias}`), el('span', { class: 'vida' }, corazones)), el('div', { class: 'estado', html: estado }));
  if (T.levantandose) { panel.append(el('p', {}, `${j.nombre} estaba en el suelo. Se levanta con 1 de vida y pierde el turno.`), el('button', { class: 'primario', onclick: () => { terminarTurno(G); tras(); } }, 'Continuar')); dibujar(); root.querySelector('.mano').innerHTML = ''; return; }
  const dados = el('div', { class: 'dados' }, ...T.dados.map((d, i) => el('button', { class: 'dado ' + d, title: d, onclick: () => ordenar(i) }, { paso: '→', doble: '⇒', ruido: '♪', mordisco: '☠' }[d])));
  const elias = G.jugadores.find(x => x.personajeId === 'elias' && x.estado === 'vivo'); const puedeOrden = elias && !G.banderas.ordenUsada && elias.id !== j.id && dist(elias.pos, j.pos) <= 3 && j.personajeId !== 'elias';
  ...
  // acciones
  const acc = el('div', { class: 'acciones' }); const c = G.casillas[j.pos]; const zs = zombisEn(G, j.pos);
  const puedeSaquear = ['edificio', 'gasolinera', 'farmacia', 'taller'].includes(c.tipo) && c.saqueos < PARAMS.saqueosPorCasilla && !zs.length;
  const btn = (t, f, dis = false, cls = '') => acc.append(el('button', { class: cls, disabled: dis || T.acciones <= 0 && !cls.includes('libre'), onclick: f }, t));
  ...
  const recetas = G.recetasConocidas.filter(r => RECETAS[r].ing.every(ing => j.mano.some(cc => cc.id === ing) && RECETAS[r].ing.filter(x => x === ing).length <= j.mano.filter(cc => cc.id === ing).length));
  btn(`Craftear${c.tipo === 'taller' ? ' (gratis)' : ''}`, () => elegirReceta(), false);
  btn(j.vehiculo ? 'Bajar del vehículo' : 'Subir a vehículo', () => { if (j.vehiculo) return tras(vehiculo(G)); const vs = j.mano.filter(cc => OBJETOS[cc.id].porGas); if (vs.length === 1) tras(vehiculo(G, vs[0].uid)); else elegirDeLista('¿A cuál?', vs.map(v => [OBJETOS[v.id].nombre + ' · gas ' + v.gas, () => tras(vehiculo(G, v.uid))])); }, !j.vehiculo && !j.mano.some(cc => OBJETOS[cc.id].porGas));
  const adyacentes = vivos(G).filter(x => x.id !== j.id && dist(x.pos, j.pos) <= 1);
  btn('Dar cartas', () => darCartas(adyacentes), !adyacentes.length || !j.mano.length);
  ...
  acc.append(el('button', { class: 'primario libre', onclick: () => { terminarTurno(G); UI.modo = 'mover'; tras(); } }, 'Terminar turno'));
  panel.append(acc, el('p', { class: 'muted small ayuda' }, UI.modo === 'mover' ? (T.pasos > 0 || j.vehiculo ? 'Toca una casilla iluminada para moverte. Toca un zombi para atacarlo.' : 'Sin pasos. Usa tus acciones o termina el turno.') : UI.modo === 'molotov' ? 'Toca la casilla adyacente donde lanzar el molotov.' : 'Toca una loseta oculta adyacente para rastrearla.'));
  // mano
...
function ordenar(i) { const r = repetirDado(G, i, null); if (!r.ok) { toast(r.motivo || 'No se puede repetir'); return; } tras(); }
function elegirDeLista(titulo, items) { const c = el('div', { class: 'lista' }); items.forEach(([t, f]) => c.append(el('button', { onclick: () => { $('#modal').hidden = true; f(); } }, t))); modal(titulo, c, [['Cancelar', null]]); }
function elegirAtaque(objs) {
  const j = turnoActual(G); const armas = armasDe(G, j);
  const c = el('div', { class: 'lista' });
  objs.forEach(({ z, d }) => { const nombre = (z.tipo === 'jugador' ? G.jugadores[z.jugadorId].nombre + ' (zombi)' : ZOMBIS[z.tipo].nombre) + (z.n > 1 ? ' ×' + z.n : '') + (d ? ` · a ${d} casillas` : ' · aquí');
    const opciones = [[null, 'Manos (1 dado)'], ...armas.filter(a => d === 0 || OBJETOS[a.id].distancia).filter(a => !(d > 0 && ZOMBIS[z.tipo].inmuneDistancia)).map(a => [a.uid, `${OBJETOS[a.id].nombre} (${OBJETOS[a.id].dados} dados${OBJETOS[a.id].fuego ? ', ruido' : ''})`])];
    if (d > 0) opciones.shift();
    c.append(el('div', { class: 'objetivo' }, el('b', {}, nombre), el('div', {}, ...opciones.map(([uid, t]) => el('button', { onclick: () => { $('#modal').hidden = true; const r = atacar(G, z.id, uid); if (r.ok) toast(`[${r.dados.join(' ')}] ${r.impactos} impactos · ${r.muertos} eliminados${r.mordido ? ' · ¡te muerde!' : ''}`); tras(r); } }, t))))); });
  modal('Atacar', c, [['Cancelar', null]]);
}
function elegirReceta() {
  const j = turnoActual(G); const c = el('div', { class: 'lista' });
  G.recetasConocidas.forEach(r => { const R = RECETAS[r]; const tiene = R.ing.every(ing => j.mano.some(cc => cc.id === ing)); c.append(el('button', { class: 'receta' + (tiene ? '' : ' falta'), disabled: !tiene, onclick: () => { $('#modal').hidden = true; tras(craftear(G, r)); } }, el('b', {}, R.nombre), el('span', {}, R.ing.map(i => OBJETOS[i].nombre).join(' + ')), el('small', {}, R.texto))); });
  if (!G.recetasConocidas.length) c.append('No conoces recetas.'); modal('Libro de recetas', c, [['Cerrar', null]]);
}
function darCartas(ady) { const j = turnoActual(G); const c = el('div', { class: 'lista' }); const sel = new Set(); j.mano.forEach(cc => c.append(el('label', { class: 'check' }, el('input', { type: 'checkbox', onchange: e => e.target.checked ? sel.add(cc.uid) : sel.delete(cc.uid) }), OBJETOS[cc.id].nombre))); const dest = el('select', {}, ...ady.map(x => el('option', { value: x.id }, x.nombre))); c.append(el('label', {}, 'Para ', dest)); modal('Dar cartas (1 acción)', c, [['Dar', () => tras(dar(G, +dest.value, [...sel]))], ['Cancelar', null]]); }
function usarCarta(cc) {
  ...
  if (O.porGas && !j.vehiculo) ops.push(['Subir', () => tras(vehiculo(G, cc.uid))]);
  const desc = cc.id in RECETAS ? RECETAS[cc.id].texto : O.tipo === 'arma' ? `${O.dados} dados${O.extra ? ' +' + O.extra : ''}${O.distancia ? ', distancia ' + O.distancia : ', cuerpo a cuerpo'}${O.fuego ? ', +2 ruido' : ''}${O.mataCorredor ? ', mata corredores' : ''}.` : O.tipo === 'material' ? 'Material de crafteo.' : O.tipo === 'vehiculo' && !O.porGas ? 'Necesita gasolina: craftea con un bidón.' : cc.id === 'comida' ? 'Se deposita al terminar turno en el refugio.' : '';
...
function pintarZombi() {
  ...
  panel.append(el('div', { class: 'quien' }, el('b', {}, j.nombre + ' · zombi'), el('span', {}, ` · nivel ${G.nivelZombi} · ${G.zturno.acciones} acciones · ${G.zturno.pasos} pasos`)));
  const acc = el('div', { class: 'acciones' });
  const b = (t, f, dis) => acc.append(el('button', { disabled: dis, onclick: f }, t));
  ...
  panel.append(acc, el('p', { class: 'muted small ayuda' }, 'Ganas cuando la mitad del equipo esté convertido. Puedes hablar y mentir en la mesa.'));
  root.querySelector('.mano').innerHTML = ''; dibujar();
}

/* ---------- fin ---------- */
function fin() { mostrar('fin'); const c = $('#p-fin .cont'); c.innerHTML = ''; const F = G.fin; c.append(el('h1', { class: F.resultado }, F.resultado === 'victoria' ? 'Misión cumplida' : 'Misión fracasada'), el('p', {}, F.motivo), el('p', { class: 'muted' }, `${MISIONES[G.misionId].nombre} · ${G.ronda} rondas · ruido final ${G.ruido}`));
  const t = el('table', { class: 'expediente' }); t.append(el('tr', {}, el('th', {}, 'Superviviente'), el('th', {}, 'Personaje'), el('th', {}, 'Estado'))); G.jugadores.forEach(j => t.append(el('tr', {}, el('td', {}, j.nombre), el('td', {}, PERSONAJES[j.personajeId].nombre), el('td', { class: j.estado }, { vivo: `vivo (${j.vida} vida)`, caido: 'en el suelo', zombi: 'convertido', muerto: 'muerto' }[j.estado]))));
...
function ajustarCanvas() { const cv = canvas(); if (!cv) return; const r = cv.parentElement.getBoundingClientRect(); const dpr = Math.min(2, window.devicePixelRatio || 1); cv.width = Math.floor(r.width * dpr); cv.height = Math.floor(r.height * dpr); cv._dpr = dpr; if (!UI.cam.ajustada) { encuadrar(); UI.cam.ajustada = true; } }
...
const COL = { calle: '#4a4d47', edificio: '#6a6152', bosque: '#3f4d33', gasolinera: '#7a5a2e', farmacia: '#6e4a4a', taller: '#5b5a6a', refugio: '#7a3a2a', entrada: '#5a2d2d', helipuerto: '#3d5a6a', oculta: '#22241f' };
const ICON = { gasolinera: '⛽', farmacia: '✚', taller: '🔧', refugio: '⌂', helipuerto: 'H', entrada: '↯', bosque: '♣', edificio: '▪' };
...
function dibujar() {
  ...
  for (const c of Object.values(G.casillas)) {
    ...
    ctx.fillStyle = c.revelada ? COL[c.tipo] : COL.oculta; ctx.fill();
    ctx.lineWidth = Math.max(1, s * 0.06); ctx.strokeStyle = '#151714'; ctx.stroke();
    if (c.revelada) { if (c.fuego) { ctx.fillStyle = 'rgba(230,120,40,.7)'; ctx.fill(); } if (c.senuelo) { ctx.fillStyle = 'rgba(214,163,25,.35)'; ctx.fill(); }
      if (ICON[c.tipo]) { ctx.fillStyle = 'rgba(255,255,255,.55)'; ctx.font = `${s * 0.7}px sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(ICON[c.tipo], x, y - (zombisEn(G, c.k).length || jugadoresEn(G, c.k).length ? s * 0.45 : 0)); }
      ...
      if (c.objetos.length) { ctx.fillStyle = '#e6e3da'; ctx.font = `${s * 0.6}px sans-serif`; ctx.fillText('🚗', x, y); }
    } else { ctx.fillStyle = 'rgba(255,255,255,.12)'; ctx.font = `${s * 0.7}px sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('?', x, y); }
    // borde de loseta
    for (let i = 0; i < 6; i++) { const [dq, dr] = [[1, 0], [1, -1], [0, -1], [-1, 0], [-1, 1], [0, 1]][i]; const v = G.casillas[(c.q + dq) + ',' + (c.r + dr)]; if (v && v.loseta === c.loseta) continue; const a1 = Math.PI / 6 + (i - 1) * Math.PI / 3, a2 = a1 + Math.PI / 3; }
  }
  // límites de loseta (línea más clara entre losetas distintas)
  ...
  // resaltes
  for (const k of [...dest, ...zdest]) { const [px, py] = pixel(k, s); hexPath(ctx, ox + px, oy + py, rr - s * 0.08); ctx.lineWidth = Math.max(2, s * 0.12); ctx.strokeStyle = dest.has(k) ? '#9db060' : '#c9553d'; ctx.stroke(); }
  ...
  // zombis
  for (const z of Object.values(G.zombis)) { const c = G.casillas[z.pos]; if (!c.revelada) continue; const [px, py] = pixel(z.pos, s); const x = ox + px + s * 0.3, y = oy + py + s * 0.25; const r = s * (esHorda(z) ? 0.42 : 0.32);
    ctx.beginPath(); ctx.arc(x, y, r, 0, Math.PI * 2); ctx.fillStyle = z.tipo === 'jugador' ? '#8fa86a' : z.tipo === 'acorazado' ? '#3a3f4a' : z.tipo === 'corredor' ? '#7a5a3a' : z.tipo === 'nino' ? '#6a5a6a' : '#4f5a3a'; ctx.fill(); ctx.lineWidth = 2; ctx.strokeStyle = '#c9553d'; ctx.stroke();
  ...
  // jugadores
  for (const j of G.jugadores) { if (j.estado === 'zombi' || j.estado === 'muerto') continue; const [px, py] = pixel(j.pos, s); const compa = jugadoresEn(G, j.pos); const i = compa.indexOf(j); const ang = compa.length > 1 ? (i / compa.length) * Math.PI * 2 : 0; const off = compa.length > 1 ? s * 0.32 : 0;
    ...
    ctx.fillStyle = '#151714'; ctx.font = `bold ${r * 1.1}px sans-serif`; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(j.estado === 'caido' ? '✕' : String(j.id + 1), x, y); if (j.mordido) { ctx.fillStyle = '#c9553d'; ctx.beginPath(); ctx.arc(x + r * 0.8, y - r * 0.8, r * 0.4, 0, Math.PI * 2); ctx.fill(); } }
}
function casillaEn(cx, cy) { const cv = canvas(); const { s, x: ox, y: oy } = UI.cam; const dpr = cv._dpr || 1; const x = (cx * dpr - ox) / s, y = (cy * dpr - oy) / s; const r = y / 1.5, q = x / Math.sqrt(3) - r / 2; // redondeo cúbico
  let rq = Math.round(q), rr = Math.round(r), rs = Math.round(-q - r); const dq = Math.abs(rq - q), dr = Math.abs(rr - r), ds = Math.abs(rs - (-q - r)); if (dq > dr && dq > ds) rq = -rr - rs; else if (dr > ds) rr = -rq - rs; const k = rq + ',' + rr; return G.casillas[k] ? k : null; }
function tocar(k) {
  if (!k) return; const c = G.casillas[k];
  if (G.fase === 'turno') {
    const j = turnoActual(G);
    if (UI.modo === 'molotov') { const m = j.mano.find(cc => cc.id === 'molotov'); UI.modo = 'mover'; tras(usar(G, m.uid, k)); return; }
    ...
    if (UI.modo === 'zmover') { const r = zMover(G, k); tras(r); return; }
...
function info(k) { const c = G.casillas[k]; if (!c.revelada) { modal('Loseta oculta', '<p>Se revela al entrar. Puede haber zombis.</p>'); return; } const zs = zombisEn(G, k).map(z => `${ZOMBIS[z.tipo].nombre}${z.n > 1 ? ' ×' + z.n : ''}${esHorda(z) ? ' (horda)' : ''}`); const js = jugadoresEn(G, k).map(j => `${j.nombre} (${PERSONAJES[j.personajeId].alias})`);
  const tipo = { calle: 'Calle. Los vehículos circulan por aquí.', edificio: 'Edificio. Se puede saquear dos veces. Los zombis tardan en entrar.', bosque: 'Bosque. Cuesta 2 pasos. Bloquea la visión.', gasolinera: 'Gasolinera. Saquear da bidones y hace ruido.', farmacia: 'Farmacia. Botiquín y antibióticos, dos saqueos.', taller: 'Taller. Craftear aquí es gratis. Piezas y recetarios.', refugio: 'Refugio. Cura 1 al final de la ronda. La comida y los antibióticos se depositan aquí.', entrada: 'Entrada de horda.', helipuerto: 'Helipuerto. Objetivo de Sin una gota.' }[c.tipo];
  modal(c.tipo[0].toUpperCase() + c.tipo.slice(1), `<p>${tipo}</p>${c.saqueos ? `<p class="muted">Saqueada ${c.saqueos} vez/veces.</p>` : ''}${c.fuego ? `<p class="peligro">Arde ${c.fuego} rondas.</p>` : ''}${c.senuelo ? `<p>Señuelo activo ${c.senuelo} rondas.</p>` : ''}${zs.length ? `<p><b>Zombis:</b> ${zs.join(', ')}</p>` : ''}${js.length ? `<p><b>Supervivientes:</b> ${js.join(', ')}</p>` : ''}`); }
...
$('#modal-fondo').addEventListener('click', e => { if (e.target.id === 'modal-fondo') $('#modal').hidden = true; });
if (cargar() && G) reanudar(); else inicio();
